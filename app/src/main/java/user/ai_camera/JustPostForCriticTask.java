...
public class JustPostForCriticTask extends AsyncTask<String, Void, String> {
	...
	public JustPostForCriticTask(Drawable img, float[] direction, String keyword,String authentication){
		this.img = img;
	...
	protected String doInBackground(String... urls) {
		//Log.d("JTask direction test in background", String.valueOf(direction[0]));
	...
	protected void onPostExecute(String result) {
		//Log.d("JTask End Of Execute", "processing json ");
		Log.d("JTask End Of Execute", "processing json result: " + result);
		try {
			JSONObject jObject = new JSONObject(result);

			Log.d("JTask Results Score", jObject.getJSONArray("results").getJSONObject(0).getString("score"));
			...
			isJSONEmpty = jObject.getJSONArray("results").getJSONObject(0).length() == 0;
	...
	private String POST(String url) {
		...
		try {
			...
			Log.d("JTask InputStream", e.getLocalizedMessage());
		}
		return result;
	}
